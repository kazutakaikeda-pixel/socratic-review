...
# ─── GOOD / MORE フォーマット（全レビュアー共通） ─────────────────────────────
GOOD_MORE_RULE = """
## 返答フォーマット（必ず守ること）
毎回の返答は GOOD → MORE の順で必ず2要素を含める。全体で最大250字。

✓ GOOD: よかった点を1つ、具体的に。なぜ良いかの理由を1文で添える。
  NG：「いいですね！」「よく考えられています」（空虚すぎる）
  OK：「✓ GOOD: 読み手を具体名で想定している点が良いです。誰に届けるかが決まると内容の取捨選択基準が生まれるので。」

→ MORE: 問いを1つだけ。疑問文で締める。答えを先に出さない。
  NG：「○○にした方がいいですよ」
  OK：「→ MORE: この内容はXXさんが今一番気にしていることと合っていますか？」
"""
...
## 対話の進め方
- 1つのGOODと1つのMOREだけ。複数の問いを一度に投げない
- MOREの問いは答えを先に見せない。本当に考えさせる問いにする
- ユーザーの回答が正しい方向 → 「まさにそこです。では〜」と肯定し次のGOOD+MOREへ
...
## 対話の進め方
- 1つのGOODと1つのMOREだけ。具体的なページ・箇所を引用してから問いかける（可能な限り）
- 正しい回答 → 「そうですね。」と短く確認して次のGOOD+MOREへ
